...
func TestBenchmark(t *testing.T) {
	...
	t1 := time.Now().Unix()
	randomTable.AddColumn("Col1", INT).AddColumn("Col2", INT)
	for i := 0; i < 100; i++ {
		for j := 0; j < 200; j++ {
			randomTable.Insert([]interface{}{i, j})
		}
	}
	...
	t1 = time.Now().Unix()
	for i := 0; i < 10; i++ {
		c1 := rand.Intn(100-0) + 0
		c2 := rand.Intn(200-0) + 0
		randomTable.Where(fmt.Sprintf(":Col1 == %d && :Col2 == %d", c1, c2))
	}
